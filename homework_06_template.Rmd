...
\mu_{0} -2\tau_{0}=10 \\
\mu_{0}=11, \tau_{0}=0.5 \\
\mu \mid \mu_0, \tau_0 \sim \mathrm{normal} \left( \mu \mid 11, 0.5 \right)
$$
...
my_logpost <- function(unknowns, my_info)
{
  ...
  log_lik <- 8*log(1/(sqrt(2*pi*lik_sigma^2)))+ (-1/(2*lik_sigma^2))*sum((my_info$xobs-lik_mu)^2)
  
  # calculate the log-prior on mu
  log_prior_mu <- log(1/(sqrt(2*pi*my_info$tau_0^2)))+ (-1/(2*my_info$tau_0^2))*(lik_mu-my_info$mu_0)^2
...
sigma_grid_lwr <- 1
sigma_grid_upr <- qnorm(0.99, 11,0.5)
```
...
param_grid %>% 
  ...
  geom_point(data = tibble::tibble(xbar = x_mean, xsd =x_sdd ),
  ...
  theme_bw()
```
The sample average and standard deviation located in the contour line that represent top 50% of $\mu$ and $\sigma$ combination, while the posterior located in the center of the top 100% of $\mu$ and $\sigma$ combination.The sample average is a less than 10.5, while the mode $\mu$ is close to 11. So they are not similiar. The posterior uncertainty in $\mu$ is the horizontal width, and the posterior uncertainty in $\sigma$ is the vertical height. There are a high uncertainties in posterior $\mu$ and $\sigma$.
...
Based on previous homework, this is the first derivative of un-normalized posterior
$$
\frac{N}{\sigma^{2}} ( \bar{x}-\mu)-\frac{1}{\tau_{0}^{2}}(\mu-\mu_{0})
$$
The second derivative equals:
$$
\frac{d}{d\mu} \frac{N}{\sigma^{2}} ( \bar{x}-\mu)-\frac{d}{d\mu} \frac{1}{\tau_{0}^{2}}(\mu-\mu_{0})= \\
...
Because after getting the second derivative, we found that the second derivative will be less than 0, given all other constances have to be equal or greater than 0. When second derivative is less than 0, the curve concave down, and the correspond $\mu$ is the local maximum value. 
...
$$
\frac{1}{S_{N}^2}=\frac{N}{\sigma^2}+\frac{1}{\tau_0^2}
$$

$$
m_{N}=\frac{\frac{1}{\sigma^2}}{\frac{1}{\tau_{0}^2}+\frac{N}{\sigma^{2}}}\sum_{n=1}^{N} {x}+\frac{\frac{1}{\tau_{0}^{2}}}{\frac{1}{\tau_{0}^2}+\frac{N}{\sigma^{2}}} \mu_{0}
$$
...
What do you think?  
The Laplace Approximation equal to the exact posterior distribution is because it is a 1-Dimensional density function. There's no distance between variables, since there's only one variable. The Laplace Approximation therefore equal to the exact posterior distribution.
## Problem 03
...
the likelihood noise, $\sigma$, has a natural lower bound of 0. A Gaussian distribution has no such constraints, and so allows all values from negative infinity to positive infinity! Therefore, even though the prior Exponential distribution respects the bound on $\sigma$, he lower bound would be violated if we apply the Laplace Approximation to approximate the posterior!  
...
$$
p \left( \mu, \varphi \mid \mathbf{x} \right) \propto \prod_{n=1}^{N} \left( \mathrm{normal} \left(x_n \mid \mu, \varphi \right) \right) \times \mathrm{normal}\left(\mu \mid \mu_0, \tau_0\right) \times \mathrm{Exp}\left(\varphi \mid \lambda=0.5\right)
$$
...
$$
\frac{d\sigma}{d\varphi}=e^{\varphi} (1-e^{\varphi})
$$
...
my_cv_logpost <- function(unknowns, my_info)
{
  ...
  # return the (un-normalized) log-posterior
  return(log_lik+log_prior_mu+log_prior_sigma+lik_varphi)
}
```
...
my_cv_logpost(test_1, hw06_info)
```
...
my_cv_logpost(test_2, hw06_info)
```
...
map_res_02
```
The identified optimal parameter values are the same. The Hessian matrices are the same. Two things are different: the count of function and gradient. Second guess has more count, which means it takes more steps to MAP.

The location and position of two initial guess points in log-posterior surface gives hint about the two different results. The second guess located outside of the 99% contour line while the first guess located between 90% and 99% contour line.The first guess also locate in part where the color is more yellow and second guess locates in a bluer shade. These mean that the distance from first guess to MAP is closer than the second guess to MAP. 
...
The approximate posterior mean is 10.855893  and the approximate posterior standard deviation is 1.090289. The posterior correlation coefficient between $\mu$ and $\varphi$ is -0.01291656.
...
inig=c(x_mean,log(x_sdd))
laplace_result1 <-  my_laplace(inig, my_logpost,hw06_info)
laplace_result1
```


Include as many code chunks and discussion text as you feel are necessary.  
Since sample average is 10.218
