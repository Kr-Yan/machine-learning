...
```{r}
quad_data_big<-tibble::rowid_to_column(quad_data_big, var="rowid")
ggplot(data=quad_data_big,aes(x=x, y=y))+geom_point(aes(color=rowid>5))
```
...
Xmat_quad_small <- model.matrix(y~x+x^2, quad_data_small)
```


```{r, solution_for_1b_b, eval=TRUE}
### make the design matrix for the BIG data
Xmat_quad_big <- model.matrix(y~x+x^2, quad_data_big)
```
...
Xmat_quad_small
```
the design matrix for the big dataset contains 30 rows and two columns, while the design matrix for the small dataset has 5 rows and 2 columns. They first column contains the intercept value.
...
There are two rows and two columns that each posterior covariance matrix have.
...
```{r}
#standard deviation on the big dataset
sqrt(diag(covmat_quad_big_noprior))

#standard deviation on the small dataset
sqrt(diag(covmat_quad_small_noprior))
```

For big dataset, the standard deviation of intercept is 0.1825745, and the standard devation of x is 0.1939791.
For small dataset, the standard deviation of intercept is 0.5042698, and the standard devation of x is 0.6969201.
### 1g)
...
x_quad_small_col <- quad_data_small %>% pull(x) %>% as.matrix()
postmeans_quad_small_noprior <- solve(t(x_quad_small_col) %*% x_quad_small_col) * (t(x_quad_small_col)%*% y_quad_small_col)
...
x_quad_big_col <- quad_data_big %>% pull(x) %>% as.matrix()
postmeans_quad_big_noprior <- solve(t(x_quad_big_col) %*% x_quad_big_col) * (t(x_quad_big_col)%*% y_quad_big_col)
...
The value won't change if $\sigma$ changes.
...
```{r}
interval <- c(postmeans_quad_small_noprior-2*1, postmeans_quad_small_noprior +2*1 )
interval 
```
...
covmat_quad_big_vague <- solve(solve(B0_inform)+(t(Xmat_quad_big) %*% Xmat_quad_big)/(1^2))
```
...
as.matrix(quad_data_big[,1:ncol(quad_data_big)-1])
```
...
lm_logpost <- function(unknowns, my_info)
{
  ...
  length_beta <- as.numeric(unknowns[length(unknowns)])-1
  
  # extract the beta parameters from the `unknowns` vector
  beta_v <- as.vector(unknowns[1:length(unknowns)-1])
  
  # extract the unbounded noise parameter, varphi
  lik_varphi <- as.numeric(unknowns[4])
  ...
  log_lik <- (-1/2)*log(lik_varphi^2) - (1/2)*log(2*pi)+(-1/2*lik_varphi^2)*t(my_info$yobs-mu)*(my_info$yobs-mu)
  
  # evaluate the log-prior
  log_prior_beta <- log(1/(sqrt(2*pi*my_info$tau_beta^2)))+ (-1/(2*my_info$tau_beta^2))*(as.vector(beta_v)-my_info$mu_beta)^2
  ...
  log_derive_adjust <- log_prior+lik_varphi
...
#as.numeric(a[1:length(a)-1])
as.vector(a[1:length(a)-1])
```
