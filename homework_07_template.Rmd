...
Xmat_quad_small <- model.matrix(y~x+I(x^2), quad_data_small)
```


```{r, solution_for_1b_b, eval=TRUE}
### make the design matrix for the BIG data
Xmat_quad_big <- model.matrix(y~x+I(x^2), quad_data_big)
```
...
(t(Xmat_quad_small) %*% Xmat_quad_small)
```
...
###
laplace_quad$mode
sqrt(laplace_quad$var_matrix[1,1])
sqrt(laplace_quad$var_matrix[2,2])
sqrt(laplace_quad$var_matrix[3,3])
sqrt(laplace_quad$var_matrix[4,4])
```
...
```{r, solution_03d_c}
###
ggplot(post_samples_quad, aes(x=beta_02))+ geom_histogram(bins = 55)
```
...
30X2500. There are 30 rows and 2500 columns
...
?  
we are averaging over the length of the error vector. The MAE are not the same, because there are difference in posterior of randomly generated samples. 
...
absError_quad_mat <- abs(quad_data_big$y-post_trend_samples_quad)
...
Because the posterior samples are randomly generated, so the variance are uncertain. So is MAE.
...
```{r solution_05a}
ggplot(train_df,aes(x,y)) +geom_point() +geom_point(data=test_df,colour='red') 
```
