...
N=(\frac{1}{\tau_{n}^2}- \frac{1}{\tau_{0}^2})\times \sigma^2\\
$$
When $\tau_{N} = 2$:
$$
N=(\frac{1}{2^2}- \frac{1}{5^2})\times 5^2 \\
\lceil N \rceil=\frac{21}{4}=6
$$
When $\tau_{N} = 0.5$:
$$
N=(\frac{1}{\frac{1}{2}^2}- \frac{1}{5^2})\times 5^2 \\
N=(4-\frac{1}{5^2})\times 5^2 \\
N=99
$$
...
\frac{1}{\tau_{n}^2}= \frac{1}{\tau_{0}^2} + \frac{N}{\sigma^2} \\
N=(\frac{1}{\tau_{n}^2}- \frac{1}{\tau_{0}^2})\times \sigma^2\\
N= \frac{\sigma^2}{\tau_{n}^2}- \frac{\sigma^2}{\tau_{0}^2}
$$
Since $\tau_{0} \rightarrow \infty$, $\frac{\sigma^2}{\tau_{0}^2}\rightarrow 0$

$$
N= \frac{15^2}{3^2}- \frac{15^2}{\infty^2}\\
N= \frac{225}{9}-0\\
N= 25
$$
...
=\log\left(\prod_{n=1}^{N} \{ e^{-\frac{1}{2\sigma ^{2}} (x_n-\mu)^2} \} \times \left( e^{-\frac{1}{2\tau_{0} ^{2}} (\mu-\mu_{0})^2} \right)\right)\\
=\sum_{n=1}^{N}\log [ e^{-\frac{1}{2\sigma ^{2}} (x_n-\mu)^2} ]+ \log \left[\left( e^{-\frac{1}{2\tau_{0} ^{2}} (\mu-\mu_{0})^2} \right)\right]\\
=\frac{-1}{2\sigma^{2}}\sum_{n=1}^{N}\{(x_{n}-\mu)^2\}- \frac{1}{2\tau_{0}^{2}}(\mu-\mu_{0})^{2} 
$$
$$
\log\left[p\left(\mu \mid \mathbf{x}, \sigma\right)\right] \propto \frac{-1}{2\sigma^{2}}\sum_{n=1}^{N}\{(x_{n}-\mu)^2\}- \frac{1}{2\tau_{0}^{2}}(\mu-\mu_{0})^{2} 
$$
...
You may add as many equation blocks as you see fit to answer this question.  
$$
\dfrac{d}{d\mu}\left(\frac{-1}{2\sigma^{2}}\sum_{n=1}^{N}\{(x_{n}-\mu)^2\}- \frac{1}{2\tau_{0}^{2}}(\mu-\mu_{0})^{2}\right)= \\
\frac{-1}{2\sigma^{2}}\sum_{n=1}^{N}\dfrac{d}{d\mu}\{(x_{n}-\mu)^2\}- \frac{1}{2\tau_{0}^{2}}\dfrac{d}{d\mu}(\mu-\mu_{0})^{2} = \\
\frac{-1}{2\sigma^{2}}\sum_{n=1}^{N}\dfrac{d}{d\mu}\{(x_{n}-\mu)^2\}- \frac{1}{\tau_{0}^{2}}(\mu-\mu_{0})
$$
For Sum of N derivatives, we starts from N-th observations: 
$$
\dfrac{d}{d\mu} (x_{n}-\mu)^2=  -2(x_{n}-\mu)
$$
N-Derivatives 
$$
\sum_{n=1}^{N} (-2(x_{n}-\mu)) = -2 \sum_{n=1}^{N} (x_{n}-\mu)=-2 \left(\sum_{n=1}^{N} x_{n}- \sum_{n=1}^{N}\mu\right)
$$
$$
Since \sum_{n=1}^{N} \mu= \mu\sum_{n=1}^{N}(1) =\mu \times N\\
Since \sum_{n=1}^{N} x_{n}= \bar{x} N\\
-2 \left(\sum_{n=1}^{N} x_{n}- \sum_{n=1}^{N}\mu\right) =-2( \bar{x} N-\mu N)
$$
To sum up
$$
\frac{-1}{2\sigma^{2}}\sum_{n=1}^{N}\dfrac{d}{d\mu}\{(x_{n}-\mu)^2\}- \frac{1}{\tau_{0}^{2}}(\mu-\mu_{0})=\\
\frac{-1}{2\sigma^{2}} \times -2( \bar{x} N-\mu N)-\frac{1}{\tau_{0}^{2}}(\mu-\mu_{0})=\\
\frac{N}{\sigma^{2}} ( \bar{x}-\mu)-\frac{1}{\tau_{0}^{2}}(\mu-\mu_{0})
$$
...
$$
\frac{N}{\sigma^{2}} ( \bar{x}-\mu)-\frac{1}{\tau_{0}^{2}}(\mu-\mu_{0})=0\\
\frac{N}{\sigma^{2}} ( \bar{x}-\mu)=\frac{1}{\tau_{0}^{2}}(\mu-\mu_{0})\\
\mu=\frac{\frac{N}{\sigma^2}}{\frac{1}{\tau_{0}^2}+\frac{N}{\sigma^{2}}}\bar{x}+\frac{\frac{1}{\tau_{0}^{2}}}{\frac{1}{\tau_{0}^2}+\frac{N}{\sigma^{2}}} \mu_{0}
$$
...
We referred it in lecture as posterior mean. The reason that posterior mode and posterior mean are the same expression is because that in Gaussian distribution, mode equals mean. And since the conjugate posterior of Gaussian is still Gaussian, the posterior mean equals posterior mode
...
$$
p \left(\sigma \mid \mathbf{x}, \mu\right) \propto \prod_{n=1}^{N} \left(\mathrm{normal}\left( x_n \mid \mu, \sigma \right) \right) \times  \mathrm{Exp}\left( \sigma \mid \lambda \right)\\
\log[p \left(\sigma \mid \mathbf{x}, \mu\right)] \propto \log\left[\prod_{n=1}^{N} \left(\mathrm{normal}\left( x_n \mid \mu, \sigma \right) \right) \times  \mathrm{Exp}\left( \sigma \mid \lambda \right)\right] \\ 
$$

$$
\log\left[\prod_{n=1}^{N} \left(\mathrm{normal}\left( x_n \mid \mu, \sigma \right) \right) \times  \mathrm{Exp}\left( \sigma \mid \lambda \right)\right]=\\
\log\left[ \prod_{n=1}^{N} \{\frac{1}{\sqrt{2\pi\sigma^2}}e^{\frac{-1}{2\sigma^2}(x_{n}-\mu)^{2}}\} \times \lambda e^{-\lambda\sigma} \right]=\\
\log \sum_{n=1}^{N} \{\frac{1}{\sqrt{2\pi\sigma^2}}e^{\frac{-1}{2\sigma^2}(x_{n}-\mu)^{2}}\} + \log(\lambda e^{-\lambda\sigma}) =\\
N*\log\left(\frac{1}{\sqrt{2\pi\sigma^2}}\right) - {\frac{1}{2\sigma^2} \sum_{n=1}^{N} (x_{n}-\mu)^{2}} + \log(\lambda)-\lambda\sigma
$$
$$
log[p \left(\sigma \mid \mathbf{x}, \mu\right)] \propto N*\log\left(\frac{1}{\sqrt{2\pi\sigma^2}}\right) - {\frac{1}{2\sigma^2} \sum_{n=1}^{N} (x_{n}-\mu)^{2}} -\lambda\sigma
$$
...
Add as many equation blocks as you see fit to answer this question.  
$$
N*\log\left(\frac{1}{\sqrt{2\pi\sigma^2}}\right) - {\frac{1}{2\sigma^2} \sum_{n=1}^{N} (x_{n}-\mu)^{2}} -\lambda\sigma=\\
N*\log\left(\frac{1}{\sqrt{2\pi\sigma^2}}\right) - \frac{vN}{2\sigma^2}  -\lambda\sigma
$$
Sufficient atcs as an “information content” of the observations. $v$ is referred as sufficient statistics because it is a sample mean and after making use of $v$, posterior expression no longer depends on individual observations.
...
{result<- N*log(1/sqrt(2*pi*(sigma)^2))-v*N/(2*(sigma)^2)-lambda*sigma
return(result)
}
```
...
The log-posterior has one mode, and the shape of it doesn't look like a parabola.
...
$$
 \sigma= g^{-1} (\varphi) = e^{\varphi}
$$
Expression for calculating $\sigma$ as a function of $\varphi$:
$$
  \log \left( p_{\sigma} \left( g^{-1} \left(\varphi \right) \mid \mathbf{x}, \mu \right) \right)=\\
\log\left(\frac{N}{\sqrt{2\pi e^{2\varphi}}}\right) - \frac{Nv}{2e^{2\varphi}}  -\lambda e^{\varphi}
$$
Derivative of the inverse link:
$$
 \left| \frac{d}{d\varphi} \left( g^{-1}\left(\varphi\right) \right) \right|=\\
\left| \frac{d}{d\varphi} \left( e^{\varphi} \right) \right|= e^{\varphi}
$$
Log of the derivative of the inverse link function
$$
\log \left( \left| \frac{d}{d\varphi} \left( g^{-1}\left(\varphi\right) \right) \right| \right)=\\
\log \left( \left|  \left( e^{\varphi} \right) \right| \right)= \varphi
$$
### 3f)
...
log_post_varphi_unnorm <- function(varphi,N,v,lambda)
{
  ...
  log_post <- N*log(1/sqrt(2*pi*(sigma)^2))-N*v/(2*(sigma)^2)-lambda*sigma
...
What do you think?  
The shape of log-posterior appear more parabolic.
...
fnscale tells `optim()` to maximize instead of minimize. gr sets to NULL
...
varphi_opt_b
```
The posterior mode from the initial guess of 1.35 is not different from the initial guess of -0.5
...
tibble::tibble(
  ...
  mutate(log_post=log_post_varphi_unnorm(varphi,N,v,0.75)) %>% ggplot(aes(x=varphi, y=log_post))+geom_line()+ geom_vline(xintercept=unlist(varphi_opt_b[1]), color="red")
